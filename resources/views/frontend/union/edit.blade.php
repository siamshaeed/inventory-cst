...
<form class="mt-4" action="{{ route('union.update', $union->id) }}" method="post" >
    @csrf
    @method('put')
    <div class="mb-3 row">
        <label for="name" class="col-sm-2 col-form-label text-end">Name English</label>
        <div class="col-sm-10">
            <input type="text" id="name" value="{{$union->name}}" name="name" value="{{ old('name') }}" class="form-control" placeholder="Enter Division  Name English">
    ...
    <div class="mb-3 row">
        ...
        <div class="col-sm-10">
            <input type="text" id="name_bn" value="{{$union->bn_name}}" name="name_bn" value="{{ old('bn_name') }}" class="form-control" placeholder="Enter Division  Name Bangla">
    ...
    <div class="mb-3 row">
        ...
        <div class="col-sm-10">
            ...
            @foreach ($upazilas as $upazila)
                <option value="{{ $upazila->id }}" @if($upazila->id == $union->upazila_id) selected @endif>{{ $upazila->name }}</option>
                @endforeach
                </select>
        </div>
    </div>
