        ...
        <div class="content-container">
            ...
            <div class="header d-flex justify-content-between">
                <div class="head-left d-flex">
                    <span></span>
                    <p class="mb-0 ms-1">
                        <a href="{{ route('supplier-type.index') }}" class="link" data-bs-toggle="tooltip" title="{{ __('Supplier-Type View') }}">{{ __('Supplier-Type') }}</a>
                    </p>
                </div>
                <div class="head-right d-flex">
                    <button type="button" class="btn btn-sm btn-secondary show-modal"
                            data-route="{{ route('supplier-type.create') }}"
                            data-method="get"
                            data-action="view"
                            data-toggle="tooltip"
                            data-modal_title="Create Supplier-Type"
                            title="Create New">
                        <i class="fa fa-plus"></i>
                        Create
                    </button>
                    {{--@can('create')
                        <a href="{{ route('supplier-type.create') }}" class="ms-1 text-orange" data-bs-toggle="tooltip" title="Create New"><i class="fa fa-plus"></i> Create</a>
                    @endcan--}}
                </div>
            </div>
    ...
    <script type="text/javascript">
        $(document).ready( function () {
            $('#custom-table').DataTable({
                ...
                autoWidth: true,
                responsive: true,
                ajax: '{{ route("supplier-type.index") }}',
                columns: [
